...
void PCintPort::enable(PCintPin* p, PCIntvoidFuncPtr userFunc, uint8_t mode, void* tag) {
/* << EDesignsForge */
...
int8_t PCintPort::addPin(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc, uint8_t mode, void* tag)
/* << EDesignsForge */
{
	PCintPin* tmp;
...
/* EDesignsForge >> */
			...
			if (tmp->arduinoPin == arduinoPin) { enable(tmp, userFunc, mode, tag); return(0); }
/* << EDesignsForge */
...
/* EDesignsForge >> */
        // The PCintFunc field is not set yet at this point.
...
/* EDesignsForge >> */
	...
	enable(p, userFunc, mode, tag);
#ifdef DEBUG
        // The PCintFunc field is set in function enable()
        Serial.print("userFunc addr: "); Serial.println((int)p->PCintFunc, HEX);
#endif
/* << EDesignsForge */
 ...
 */
/* >> EDesignsForge: add support for custom callback data (parameter void* tag) */
//int8_t PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc, int mode)
int8_t PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc, int mode, void* tag)
/* << EDesignsForge */
{
	PCintPort *port;
...
/* EDesignsForge >> */
	...
	return(port->addPin(arduinoPin,userFunc,mode, tag));
/* << EDesignsForge */
}
