...
/* >> EDesignsForge: add support for custom callback data (parameter void* tag) */
	...
	static		int8_t attachInterrupt(uint8_t pin, PCIntvoidFuncPtr userFunc, int mode, void* tag);
/* << EDesignsForge */
...
protected:
	...
	public:
		...
		uint8_t		mask;
		uint8_t arduinoPin;
		/* >> EDesignsForge: add support for custom callback data */
		void*   tag;
		/* << EDesignsForge */
		PCintPin* next;
	};
/* >> EDesignsForge: add support for custom callback data (parameter void* tag) */
	...
	void 		enable(PCintPin* pin, PCIntvoidFuncPtr userFunc, uint8_t mode, void* tag);
	int8_t		addPin(uint8_t arduinoPin,PCIntvoidFuncPtr userFunc, uint8_t mode, void* tag);
/* << EDesignsForge */
